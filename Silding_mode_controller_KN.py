...
class Silding_Mode_Controller:
    def __init__(self,C,K1,K2,train_conf,t_sampling):
        ...
        self.F_hat = 0
        self.F0_TD = NTD_m(self.num_carriage,t_sampling=self.t_sampling)
        self.U_TD = NTD_m(self.num_carriage,t_sampling=self.t_sampling)
    ...
    def Update(self):
        dF0,_ = self.F0_TD.Update(self.F0)
        dA,_ = self.A_TD.Update(self.train_acceleration)
        dU,_ = self.U_TD.Update(self.U)
        dF_hat = self.dF_hat_law(dA,dF0,self.S,dU,self.M)
        self.F_hat=self.F_hat+dF_hat*self.t_sampling
        return self.F_hat

    def Sync(self,X,V,A,Xd,Vd,Ad):

        self.train_location = X
        self.train_velocity = V
        self.train_acceleration = A
        self.Ad = Ad
        self.E = X-Xd
        self.E_dot = V-Vd
